...
class Blockchain {
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
            // 1. filter genesis block
            // 2. get encoded data from block
            // 3. get elements with adress corresponding with the block data owner field
            // 4. retrieve star objects
            stars = self.chain.filter((block) => block.height > 0)
                .map((block) => JSON.parse(hex2ascii(block.body)))
                .filter((blockData) => blockData.owner === address)
    ...
    validateChain() {
        ...
        let errorLog = [];
        return new Promise(async (resolve, reject) => {
            self.chain.filter((block) => block.height > 0)
                .forEach((block) => {
                    block.validate().then().catch(() => {
                        errorLog.push("bad validation");
                    });
                    if (block.height > 0 &&
                        block.previousBlockHash !== self.chain[block.height-1].hash) {
                        errorLog.push("invalid hash")
                    }
            });
            resolve(errorLog);
        });
